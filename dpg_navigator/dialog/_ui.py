...
class DialogUIBuilder:
    ...
    def _render_entry(self, entry: FileEntry, *, relative_label: bool = False) -> None:
        ...
        tint = [255, 255, 255, self.dialog._image_transparency] if entry.is_hidden else [255, 255, 255, 255]
        ...
        with dpg.table_row(parent=self.dialog._explorer_table) as row_id:
            ...
            if self.config.allow_drag:
                drag = dpg.add_drag_payload(parent=cell_name, payload_type=self.dialog._payload_type)
                if entry.is_dir:
    ...
    def _build_ui(self) -> None:
        ...
        with dpg.window(
            ...
            self.dialog._status_label = dpg.add_text("", color=[255, 80, 80], show=False)
    ...
    def _build_themes(self) -> None:
        ...
        if self.dialog._shared_selec_theme is None:
            with dpg.theme() as self.dialog._shared_selec_theme, dpg.theme_component(dpg.mvThemeCat_Core):
                dpg.add_theme_style(dpg.mvStyleVar_SelectableTextAlign, x=0, y=0.5)
        if self.dialog._shared_size_theme is None:
            with dpg.theme() as self.dialog._shared_size_theme, dpg.theme_component(dpg.mvThemeCat_Core):
                dpg.add_theme_style(dpg.mvStyleVar_SelectableTextAlign, x=1, y=0.5)
        if self.dialog._shared_preview_active_theme is None:
            with dpg.theme() as self.dialog._shared_preview_active_theme, dpg.theme_component(dpg.mvThemeCat_Core):
                dpg.add_theme_color(dpg.mvThemeCol_Button, (70, 130, 180, 255))
                dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered, (80, 145, 200, 255))
                dpg.add_theme_color(dpg.mvThemeCol_ButtonActive, (60, 115, 160, 255))
    ...
    def _build_new_folder_bar(self) -> None:
        ...
        with dpg.group(horizontal=True, show=False) as self.dialog._new_folder_group:
        ...
        for entry in entries:
            try:
                if entry.name == "\0deep_sep":
                    ...
                    continue
                parent = os.path.dirname(entry.full_path) if entry.full_path else ""
                relative = bool(
                    self.state.search_query
                    and parent
                    and os.path.normpath(parent) != os.path.normpath(self.state.current_dir)
                )
